...
function renderConnectionAlert() {
  ...
  if (connectionMessage) {
    connectionMessage.textContent = `Last error (${latest.stage}): ${latest.message}`;
  }
  if (connectionHint) {
    connectionHint.textContent = `Check that the Operator Service is running at ${client.baseUrl} and that cross-origin requests are allowed.`;
  }
}
